...
const SiteUrls = sequelize.define(
  'SiteUrls',
  {
    id: {
      allowNull: false,
...
SiteUrls.getByUrl = async (qOpts) => {
  ...
  return SiteUrls.findOne(opts).then((result) => {
...
SiteUrls.getProductsByUrl = async (qOpts) => {
  const query =
    'select p.*, supo.orderValue from site_urls_product_order as supo inner join products as p on p.id = supo.productId where supo.siteUrlId = :siteUrlId and p.productStatus = 1 ORDER BY supo.orderValue DESC LIMIT :limit OFFSET :offSet;';
  let replacements = {
